...
{    
    public static List<(T, ICharacterState)> CreateStates<T>(this CharacterFSM<T> fsm, IWeaponAnimationStrategy _aniStrategy) where T : Enum
    {
        if (typeof(T) == typeof(ActionState))
        {
            var list = (fsm as CharacterFSM<ActionState>).CreateStates(_aniStrategy);
            return list.Select(tuple => ((T)(object)tuple.Item1, tuple.Item2)).ToList();
        }
        else if (typeof(T) == typeof(MovementState))
        {
            var list = (fsm as CharacterFSM<MovementState>).CreateStates(_aniStrategy);
            return list.Select(tuple => ((T)(object)tuple.Item1, tuple.Item2)).ToList();
        }
        else if (typeof(T) == typeof(PostureState)) {

            var list = (fsm as CharacterFSM<PostureState>).CreateStates(_aniStrategy);
            return list.Select(tuple => ((T)(object)tuple.Item1, tuple.Item2)).ToList();
        }
        return null;
    }

    public static List<(ActionState, ICharacterState)> CreateStates(this CharacterFSM<ActionState> fsm, IWeaponAnimationStrategy _aniStrategy) => new()
    {
    ...
    public static List<(MovementState, ICharacterState)> CreateStates(this CharacterFSM<MovementState> fsm, IWeaponAnimationStrategy _aniStrategy) => new()
    {
    ...
    public static List<(PostureState, ICharacterState)> CreateStates(this CharacterFSM<PostureState> fsm, IWeaponAnimationStrategy _aniStrategy) => new()
    {
...
public class CharacterFSM<T> where T : Enum
{
    ...
    public CharacterFSM(StateType value, Weapon weapon, string defaultState)
    {
